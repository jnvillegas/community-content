...
class UpdateWallpaperRequest extends FormRequest
{
    ...
    {
        return auth()->check(); // Solo usuarios autenticados pueden actualizar wallpapers
    }
    ...
    {
        $wallpaperId = $this->route('wallpaper') ? $this->route('wallpaper')->id : null;

        return [
            'title' => 'required|string|max:255',
            'slug' => 'nullable|string|unique:wallpapers,slug,' . $wallpaperId,
            'alt' => 'nullable|string',
            'src' => 'required|string',
            'is_locked' => 'boolean',
